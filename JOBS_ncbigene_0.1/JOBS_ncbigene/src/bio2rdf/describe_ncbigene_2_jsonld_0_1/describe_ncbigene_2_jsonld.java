...
package bio2rdf.describe_ncbigene_2_jsonld_0_1;
...
/**
 * Job: describe_ncbigene_2_jsonld Purpose: <br>
 * Description:  <br>
 * @author test@talend.com
 * @version 5.6.0.20141024_1545
 * @status 
 */
public class describe_ncbigene_2_jsonld implements TalendJob {
	...
	private final String jobName = "describe_ncbigene_2_jsonld";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					describe_ncbigene_2_jsonld.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(describe_ncbigene_2_jsonld.this,
	...
	public static class out7Struct implements
		...
		final static byte[] commonByteArrayLock_BIO2RDF_describe_ncbigene_2_jsonld = new byte[0];
		static byte[] commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld.length) {
					if (length < 1024
							&& commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld.length == 0) {
						commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld = new byte[1024];
					} else {
						commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_BIO2RDF_describe_ncbigene_2_jsonld, 0,
						length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_BIO2RDF_describe_ncbigene_2_jsonld) {
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				URL_string_tSparqlSelectRequest_1 = "http://bio2rdf.org/sparql"
						...
						+ java.net.URLEncoder
								.encode("select ?s  where {   ?s a ?t .   ?s <http://bio2rdf.org/ncbigene_vocabulary:x-taxonomy>	<http://bio2rdf.org/taxonomy:9606> .      filter(?t = <"
										+ context.type
										+ ">) . } limit "
										+ context.limit, "UTF-8")
				...
				for (i_tSparqlSelectRequest_1 = 0; i_tSparqlSelectRequest_1 < jsonArray_tSparqlSelectRequest_1
					...
					if (out4 != null) {
						...
						URL_string_tSparqlConstructRequest_1 = "http://bio2rdf.org/sparql"
								+ "?default-graph-uri=&query="
								+ java.net.URLEncoder
										.encode("CONSTRUCT {?s ?p ?o .}   WHERE { ?s ?p ?o . filter(?s = <"
												+ row1.uri
												+ ">) . filter(?p != rdfs:label) . }",
												"UTF-8")
								+ "&format="
						...
						row3.SparqlResults = StringUtils.replaceAllStrictly(
								row3.SparqlResults, searchStr_tReplace_2_1, ""
										+ "", false, false);
						row8.uri = row3.uri;

						row8.SparqlResults = row3.SparqlResults;
						...
						row6.uri = row8.uri;
						row6.SparqlResults = row8.SparqlResults;
							...
							for (String[] item_tRunJob_1 : childReturn_tRunJob_1) {
								...
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_4, ""
												+ "", false, false);
								String searchStr_tReplace_1_5 = "http://purl.org/dc/terms/"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_5,
										"terms_" + "", false, false);
								String searchStr_tReplace_1_6 = "bio2rdf_vocabulary:"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_6, "bm_"
												+ "", false, false);
								String searchStr_tReplace_1_7 = "_vocabulary:"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_7, "_"
												+ "", false, false);
								String searchStr_tReplace_1_8 = "x-identifiers.org"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_8,
										"x-identifiers" + "", false, false);
								String searchStr_tReplace_1_9 = "http://identifiers.org/"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_9,
										"identifiers_" + "", false, false);
								String searchStr_tReplace_1_10 = "." + "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_10, "_"
												+ "", false, false);
								String searchStr_tReplace_1_11 = "nomenclature-authority:-"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
										row4.rdf, searchStr_tReplace_1_11,
										"nomenclature-authority-" + "", false,
										false);
								String searchStr_tReplace_1_12 = "symbol:-"
										+ "";
								row4.rdf = StringUtils.replaceAllStrictly(
								...
								if (out5 != null) {
									...
									{ // start of Var scope
										...
										context.debug
											...
											out7_tmp.Body = row12.Body;
											out7_tmp.ERROR_CODE = row12.ERROR_CODE;
											out7 = out7_tmp;
										} // closing filter/reject
											// ###############################
	...
	public static void main(String[] args) {
		final describe_ncbigene_2_jsonld describe_ncbigene_2_jsonldClass = new describe_ncbigene_2_jsonld();

		int exitCode = describe_ncbigene_2_jsonldClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = describe_ncbigene_2_jsonld.class
					.getClassLoader().getResourceAsStream(
							"bio2rdf/describe_ncbigene_2_jsonld_0_1/contexts/"
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : describe_ncbigene_2_jsonld");
		}
...
/************************************************************************************************
 * 150130 characters generated by Talend Open Studio for Data Integration on the
